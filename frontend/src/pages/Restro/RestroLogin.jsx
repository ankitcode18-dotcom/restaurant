...
export default function RestroLogin() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!restroId.trim() || !password.trim()) {
      setError("Please enter Restro ID and Password");
      return;
    }
    ...
    try {
      const res = await axios.post(API_BASE_URL + "/restro/restrologin", {
        restroId,
        password,
      });
      ...
      if (res.data.status === 1) {
        ...
        await localStorage.setItem("restro", JSON.stringify(res.data.restro));
        setBranchId(res.data.restro.branchId)
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div>
            ...
            <input
              type="text"
              value={restroId || ""}
              onChange={(e) => setRestroId(e.target.value)}
              placeholder="Enter Restro ID"
